    ...
    <div class="col col-12">
      ...
      <div class="container">
        <div class="row justify-content-center">
          <h4>Your Account:</h4>
          <h3 class="default-account">{{defaultAccount}}</h3>
          <hr>
          <div class="col col-12">
          ...
          <div class="input-group input-group-icon col col-12">
            ...
            <input
              ...
              v-model="reff"
              class="form-control"
              placeholder="Your Referrer Address"
            >
          </div>
          <div class="col col-12">
...
module.exports = {
  ...
  methods: {
    checkParent(cb) {
      if (!web3.isAddress(this.reff)) {
        Swal.fire({
          ...
          text:
            "Your reffere address is not valid, please enter an ethereum valid address",
          footer: ""
        });
        return false;
      }
    ...
    checkAccountStatus() {
      this.$http.post("/check-account", { account: web3.eth.accounts[0] }).then(
        function(response) {
    ...
    checkTX(hash, reff) {
      web3.eth.getTransactionReceipt(hash, function(error, result) {
        ...
        app.$http.post("/add-referrer", data).then(
          function(response) {
